#!/usr/bin/env python3
"""Generate Dead Heads' mod menu icon: a player head, cut from the vanilla
default skin.
...
OUT = os.path.join(HERE, "src/main/resources/assets/dead-heads/icon.png")
...
def build_icon():
    """A player head, because a player head holding your death drops is what
    the mod places. Face plus hat layer, cut from the default skin."""
    skin = vanilla("entity/player/wide/steve.png")
    canvas = blank(8, 8)
    blit(canvas, skin, 0, 0, sx=8, sy=8, w=8, h=8)     # face
    blit(canvas, skin, 0, 0, sx=40, sy=8, w=8, h=8)    # hat layer over it
    return scale(canvas, 16)
